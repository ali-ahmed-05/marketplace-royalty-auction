...
pragma solidity ^0.8.4;
 ...
 import "./interfaces/IAuctionLow.sol"; 
  
  
 contract AuctionLow is Context , Ownable , IAuctionLow{ 
 ...
 // Represents an auction on an NFT 
  ...
  function createAuction(uint256 itemId ,uint256 _basePrice, uint256 _endPrice, uint256 _endingUnix, address _msgSender) public override { 
  ...
  _auctionDetail[itemId] = AuctionDetails(itemId,payable(_msgSender),_basePrice,_endPrice,address(0),_basePrice,_endingUnix,block.timestamp,false); 
  ...
  if(block.timestamp > auction.endingUnix || auction.endPrice == auction.lowestBid){ 
  auction.ended = true; 
  ...
  function bid(uint256 itemId , uint256 deduct) public payable { // msg.sender -> address parameter 
  ...
  require ( amount >= deduct , "Bid :: value out of bound");
  require ( auction.lowestBid >= msg.value + amount && auction.basePrice >= msg.value + amount && auction.endPrice <= msg.value + amount ,"please send out of bound"); 
  ...
  payable(_msgSender()).transfer(deduct);
  ...
  if(_bid[i].amount != auction.lowestBid ){ 
  pendingReturns[_bid[i].bidder] += payedBids[_bid[i].bidder][itemId]; 
  } 
  } 
  } 
  
  function getLowestBid(uint256 itemId)public override view returns(uint256){ 
  ...
  function getLowestBidder(uint256 itemId)public override view returns(address){ 
  ...
  require(auction.lowestBidder != account , "You cant withdraw"); 
  ...
  AuctionDetails memory auction = _auctionDetail[itemId]; 
  require((_msgSender == _auctionDetail[itemId].seller) || (_msgSender == _auctionDetail[itemId].lowestBidder), 'You are not authorized to conclude the auction' ); 
  ...
  delete payedBids[auction.lowestBidder][itemId]; 
  uint256 payment = auction.lowestBid * 1 wei; 
   
   
  // ERC721(auction.nftContract).transferFrom(address(this), auction.lowestBidder , _tokenId); 
